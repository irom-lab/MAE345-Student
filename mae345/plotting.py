...
def _animate(i: int, states: np.ndarray, drawables: [Line3D]) -> [Line3D]:
    ...
    drawables[0].set_xdata([pt1[0], pt2[0]])
    drawables[0].set_ydata([pt1[1], pt2[1]])
    drawables[0].set_3d_properties([pt1[2], pt2[2]])

    pt1 = state[0:3] + wRb.apply(np.array([0, 1, 0]))
    pt2 = state[0:3] + wRb.apply(np.array([0, -1, 0]))

    drawables[1].set_xdata([pt1[0], pt2[0]])
    drawables[1].set_ydata([pt1[1], pt2[1]])
    drawables[1].set_3d_properties([pt1[2], pt2[2]])
